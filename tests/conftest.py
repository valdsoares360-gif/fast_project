...
async def user(session: AsyncSession):
    password = 'bobaqui'
    user = User_Factory(password=get_password_hash(password))
    session.add(user)
...
class User_Factory(factory.Factory):
    class Meta:
        model = User

    username = factory.Sequence(lambda n: f'test{n}')
    email = factory.LazyAttribute(lambda obj: f'{obj.username}@test.com')
    password = factory.lazy_attribute(lambda obj: f'{obj.username}@test.com')
